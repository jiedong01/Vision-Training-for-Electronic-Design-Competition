...
def find_colsed_blob(blobs):
    blobs_pixels =[0]*len(blobs)
    for i in range(len(blobs)):
        blobs_pixels[i] = blobs[i].pixels()
    max_value = max(blobs_pixels)
    return blobs[blobs_pixels.index(max_value)]
...
def main():
    ...
    _area_threshold   = 1000
    _pixels_threshold = 1000
    step_pixels = 10
    ...
    while not app.need_exit():
        ...
        # 3. 通过找色块找到圆球
        blobs = img.find_blobs([threshold], area_threshold=_area_threshold, pixels_threshold=_pixels_threshold)
        if len(blobs) > 0:
            closed_blob = find_colsed_blob(blobs)
            blob_rect = closed_blob.rect()
            if abs(blob_rect[2] - blob_rect[3]) < 10:
                radius   = min(blob_rect[2], blob_rect[3])//2
                circle_y = blob_rect[1] + radius 
                circle_x = blob_rect[0] + radius
            else:              
                #取得圆的半径和圆心y坐标
                radius   = min(blob_rect[2], blob_rect[3])//2
                circle_y = blob_rect[1] + radius
                ...
                pixels_last = 0
                circle_x = 0
                step = blob_rect[2]//step_pixels
                for i in range(step):
                    rect_step = [blob_rect[0]+i*step_pixels, blob_rect[1], step_pixels, blob_rect[3]]
                    blobs_step = img.find_blobs([threshold], roi=rect_step)
                    if len(blobs_step) > 0:
                        for b in blobs_step:
                            if b.pixels() >= pixels_last:
                                pixels_last = b.pixels()
                                circle_x    = rect_step[0] + step_pixels//2
    
            img.draw_circle(circle_x, circle_y, radius, image.COLOR_BLUE)
            img.draw_cross(circle_x, circle_y, image.COLOR_BLUE)
            img.draw_rect(blob_rect[0],blob_rect[1], blob_rect[2], blob_rect[3], image.COLOR_RED);
